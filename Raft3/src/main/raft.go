...
func (rf *Raft) RequestVote(args *RequestVoteArgs) {
	...
	if rf.configNum > args.ConfigNum {
		log.Printf("Me: %d, Received outdated config in from vote request from: %d", rf.me, args.CandidateId)
		rf.currentTerm = args.Term
		rf.state = Candidate
		rf.persist()
		SendVoteReply(args.Me, &reply, rf.c)
...
func Make(serverMap []string, me int,
	persister *Persister, c *websocket.Conn, N int) *Raft {
	rf := &Raft{}
	rf.peers = serverMap
